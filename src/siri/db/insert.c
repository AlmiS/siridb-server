        ...
        uint8_t flags)
{
    ...
    ilocal->source_server = source_server;
    ilocal->flags = flags;
        ...
        siridb_pcache_t ** pcache)
{
            ...
            (n -= WEIGHT_SERIES) > 0)
    {
        ...
        if (series == NULL)
        {
            /* the series does not exist so create the series and add the points.*/
        ...
        uint8_t flags)
{
    ...
    ilocal->promise = promise;
    ilocal->siridb = siridb;
    ilocal->source_server = siridb->server;
